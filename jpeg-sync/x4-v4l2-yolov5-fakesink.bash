...
gst-launch-1.0                                                                                          \
    ...
    v4l2src device="$CAM_2" ! 'image/jpeg, width=320,height=240'                                        \
        ...
        ! queue leaky=2 max-size-buffers=100 max-size-time=0 max-size-bytes=0                           \
        ! m.sink_2                                                                                      \
    v4l2src device="$CAM_3" ! 'image/jpeg, width=320,height=240'                                        \
        ! nvv4l2decoder                                                                                 \
        ! nvvideoconvert ! 'video/x-raw(memory:NVMM),format=(string)NV12'                               \
        ! queue leaky=2 max-size-buffers=100 max-size-time=0 max-size-bytes=0                           \
        ! m.sink_3                                                                                      \
    nvstreammux name=m batch-size=4 width=640 height=640 nvbuf-memory-type=0  sync-inputs=1  batched-push-timeout="50000000" \
